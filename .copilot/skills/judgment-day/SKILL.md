...
- NO → `WARNING (theoretical)` — e.g., requires malicious manifest, renamed home dir, race condition in <1ms
...
- **Scope rule**: If you fix a pattern in one file, search for the SAME pattern in ALL other files
  touched by this change and fix them ALL. Inconsistent fixes cause unnecessary re-judge rounds.
...
4. **After the Fix Agent returns**, your IMMEDIATE next action is re-launching judges in parallel. Do NOT push or commit before re-judgment completes.
...
- `fallback-registry`, `fallback-path`, or `none` → skill cache was lost. Re-read the registry immediately.

---

## Language

- **Spanish input → Rioplatense**: "Juicio iniciado", "Los jueces están trabajando en paralelo...", "Juicio terminado — Aprobado", "Escalado — necesita revisión humana"
- **English input**: "Judgment initiated", "Both judges are working in parallel...", "Judgment complete — Approved", "Escalated — requires human review"
...
- Suspect findings (only one judge) are reported but NOT automatically fixed — triage and escalate to user
