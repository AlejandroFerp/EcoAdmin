...
test.describe("Residuos", () => {
  test(
    ...
    async ({ page }) => {
      const wastePage = new WastePage(page);
      await wastePage.loginAndGoto();
  ...
  test(
    ...
    async ({ page }) => {
      const wastePage = new WastePage(page);
      await wastePage.loginAndGoto();
      await expect(wastePage.addButton).toBeVisible();
    }
  );
  ...
  test(
    ...
    async ({ page }) => {
      const wastePage = new WastePage(page);
      await wastePage.loginAndGoto();
      await expect(wastePage.tablehead).toContainText("Codigo LER");
      await expect(wastePage.tablehead).toContainText("Cantidad");
      await expect(wastePage.tablehead).toContainText("Estado");
    }
  );
  ...
  test(
    ...
    async ({ page }) => {
      const wastePage = new WastePage(page);
      await wastePage.loginAndGoto();
      await expect(wastePage.searchBox).toBeVisible();
      await expect(wastePage.searchBox).toHaveAttribute("placeholder", /residuos/i);
    }
  );
});
