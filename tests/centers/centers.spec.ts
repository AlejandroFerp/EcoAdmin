...
test.describe("Centros", () => {
  test(
    ...
    async ({ page }) => {
      const centersPage = new CentersPage(page);
      await centersPage.loginAndGoto();
  ...
  test(
    ...
    async ({ page }) => {
      const centersPage = new CentersPage(page);
      await centersPage.loginAndGoto();
      await expect(centersPage.addButton).toBeVisible();
    }
  );
  ...
  test(
    ...
    async ({ page }) => {
      const centersPage = new CentersPage(page);
      await centersPage.loginAndGoto();
      await expect(centersPage.searchBox).toBeVisible();
      await centersPage.searchBox.fill("Busqueda test");
      await expect(centersPage.searchBox).toHaveValue("Busqueda test");
    }
  );
  ...
  test(
    ...
    async ({ page }) => {
      const centersPage = new CentersPage(page);
      await centersPage.loginAndGoto();
      await expect(centersPage.tablehead).toContainText("Nombre");
      await expect(centersPage.tablehead).toContainText("Tipo");
      await expect(centersPage.tablehead).toContainText("Ciudad");
    }
  );
