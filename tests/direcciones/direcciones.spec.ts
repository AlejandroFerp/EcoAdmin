...
test.describe("Direcciones", () => {
  test(
    ...
    async ({ page }) => {
      const direcPage = new DireccionesPage(page);
      await direcPage.loginAndGoto();
  ...
  test(
    ...
    async ({ page }) => {
      const direcPage = new DireccionesPage(page);
      await direcPage.loginAndGoto();
      await expect(direcPage.addButton).toBeVisible();
    }
  );
  ...
  test(
    ...
    async ({ page }) => {
      const direcPage = new DireccionesPage(page);
      await direcPage.loginAndGoto();
      await expect(direcPage.tablehead).toContainText("Calle");
      await expect(direcPage.tablehead).toContainText("Ciudad");
      await expect(direcPage.tablehead).toContainText("CP");
    }
  );
