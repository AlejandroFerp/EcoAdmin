...
class InformesControllerTest {
    ...
    @Autowired private TrasladoRepository trasladoRepo;
        @Autowired private RecogidaRepository recogidaRepo;
    ...
    void setUp() {
        ...
        Recogida recogidaCompletada = new Recogida();
        recogidaCompletada.setResiduo(r);
        recogidaCompletada.setCentroOrigen(cp);
        recogidaCompletada.setCentroDestino(cg);
        recogidaCompletada.setTransportista(trans);
        recogidaCompletada.setFechaProgramada(LocalDate.of(2026, 1, 10));
        recogidaCompletada.setFechaRealizada(LocalDate.of(2026, 1, 11));
        recogidaCompletada.setEstado(EstadoRecogida.COMPLETADA);
        recogidaRepo.save(recogidaCompletada);

        Recogida recogidaProgramada = new Recogida();
        recogidaProgramada.setResiduo(r);
        recogidaProgramada.setCentroOrigen(cp);
        recogidaProgramada.setCentroDestino(cg);
        recogidaProgramada.setTransportista(trans);
        recogidaProgramada.setFechaProgramada(LocalDate.of(2026, 1, 15));
        recogidaProgramada.setEstado(EstadoRecogida.PROGRAMADA);
        recogidaRepo.save(recogidaProgramada);
    ...
    @Test
    @DisplayName("GET /api/informes/estadisticas devuelve agregados por LER")
    void informeEstadisticas_devuelveAgregadosPorLer() throws Exception {
        mockMvc.perform(get("/api/informes/estadisticas")
                .param("desde", "2025-01-01")
                .param("hasta", "2027-12-31")
                .param("codigoLER", "17 04 05")
                .with(user(gestor.getEmail()).roles("GESTOR")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.columns").isArray())
                .andExpect(jsonPath("$.rows").isArray())
                .andExpect(jsonPath("$.total").value(1))
                .andExpect(jsonPath("$.rows[0][0]").value("17 04 05"))
                .andExpect(jsonPath("$.rows[0][1]").value(descripcionCanonica))
                .andExpect(jsonPath("$.rows[0][2]").value("kg"))
                .andExpect(jsonPath("$.rows[0][3]").value(100.0))
                .andExpect(jsonPath("$.rows[0][4]").value(1))
                .andExpect(jsonPath("$.rows[0][5]").value(1))
                .andExpect(jsonPath("$.rows[0][6]").value(1));
    }
